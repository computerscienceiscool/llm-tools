...
func (v *DefaultPathValidator) ValidatePath(requestedPath, repositoryRoot string, excludedPaths []string) (string, error) {
	...
	for _, excluded := range excludedPaths {
		...
		if matched {
			return "", &errors.SecurityError{
				Op:   "path_validation",
				Path: cleanPath,
				Err:  fmt.Errorf("path is in excluded list: %s", cleanPath),
			}
		}
		...
		if strings.HasPrefix(cleanPath, excluded+string(filepath.Separator)) {
			return "", &errors.SecurityError{
				Op:   "path_validation",
				Path: cleanPath,
				Err:  fmt.Errorf("path is in excluded directory: %s", excluded),
			}
		}
	}
	...
	if !strings.HasPrefix(absPath, realRoot) {
		...
		if strings.HasPrefix(relPath, "..") || strings.Contains(relPath, "../") {
			return "", &errors.SecurityError{
				Op:   "path_validation",
				Path: requestedPath,
				Err:  fmt.Errorf("path traversal detected: %s", requestedPath),
			}
		}
		return "", &errors.SecurityError{
			Op:   "path_validation", 
			Path: requestedPath,
			Err:  fmt.Errorf("path is not within repository: %s", requestedPath),
		}
	}
	...
	if err != nil {
		return "", &errors.SecurityError{
			Op:   "path_validation",
			Path: requestedPath,
			Err:  fmt.Errorf("path is not within repository: %w", err),
		}
	}
	...
	if strings.HasPrefix(relPath, "..") || strings.Contains(relPath, "../") {
		return "", &errors.SecurityError{
			Op:   "path_validation",
			Path: relPath,
			Err:  fmt.Errorf("path traversal detected: %s", relPath),
		}
	}
...
func (v *DefaultPathValidator) ValidateWriteExtension(filepath string, allowedExtensions []string) error {
	...
	return &errors.ValidationError{
		...
		Value: ext,
		Err:   fmt.Errorf("file extension not allowed: %s", ext),
	}
}
