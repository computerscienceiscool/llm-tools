...
func ExecuteOpen(filepath string, cfg *config.Config, auditLog func(cmd, arg string, success bool, errMsg string)) command.ExecutionResult {
	startTime := time.Now()
	result := command.ExecutionResult{
		Command: command.Command{Type: "open", Argument: filepath},
	}
