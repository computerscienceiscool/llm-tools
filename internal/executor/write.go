...
func ExecuteWrite(filePath, content string, cfg *config.Config, auditLog func(cmd, arg string, success bool, errMsg string)) command.ExecutionResult {
	startTime := time.Now()
	result := command.ExecutionResult{
		Command: command.Command{Type: "write", Argument: filePath, Content: content},
	}
