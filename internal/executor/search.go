...
func ExecuteSearch(query string, cfg *config.Config, searchCfg *search.SearchConfig, auditLog func(cmd, arg string, success bool, errMsg string)) command.ExecutionResult {
	startTime := time.Now()
	result := command.ExecutionResult{
		Command: command.Command{Type: "search", Argument: query},
	}
